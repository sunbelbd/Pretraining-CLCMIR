...
class ConceptualCaptionsDataset(Dataset):
    def __init__(self, ann_file, image_set, root_path, data_path, seq_len=64,
        ...
        print("Loading image db now")
        self.database = list(jsonlines.open(self.ann_file))
        print("Loading image db is done!")
    ...
    def __getitem__(self, index):
        ...
        # boxes = paddle.to_tensor(boxes.copy())
        ...
        boxes_features = paddle.to_tensor(boxes_features.copy())
        ...
        # boxes[:, [0, 2]] = paddle.where(boxes[:, [0, 2]] < 0, paddle.zeros_like(boxes[:, [0, 2]]), boxes[:, [0, 2]])
        # boxes[:, [0, 2]] = paddle.where(boxes[:, [0, 2]] > w - 1, w - 1 + paddle.zeros_like(boxes[:, [0, 2]]),
        #                                 boxes[:, [0, 2]])
        boxes[:, [1, 3]] = boxes[:, [1, 3]].clip(min=0, max=h - 1)
        # boxes[:, [1, 3]] = paddle.where(boxes[:, [1, 3]] < 0, paddle.zeros_like(boxes[:, [1, 3]]), boxes[:, [1, 3]])
        # boxes[:, [1, 3]] = paddle.where(boxes[:, [1, 3]] > h - 1, h - 1 + paddle.zeros_like(boxes[:, [1, 3]]),
        #                                 boxes[:, [1, 3]])
