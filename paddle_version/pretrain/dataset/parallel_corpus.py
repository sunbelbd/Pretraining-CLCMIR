...
class ParallelCorpus(Dataset):
    def __init__(self, ann_file, pretrained_model_name, data_path, with_mlm_task=True, tokenizer=None, seq_len=64,
                 min_seq_len=64, encoding="utf-8", on_memory=True, cache_db=False, **kwargs):
            ...
            with open(os.path.join(self.data_path, ann_file), 'r', encoding=self.encoding, errors='ignore') as f:
                for l in f.readlines():
                    ...
                    if len(corpus_sub) < max_lines:
                        ...
                        corpus_sub.append((s1, s2))
                    else:
                        idx = random.randint(0, max_lines)
                        if idx < max_lines:
                            corpus_sub[idx] = (s1, s2)
                    if i % 50000 == 0:
    ...
    #     """
    #     Used when self.corpus is concatenated dataset
    #     :param idx:
    #     :return:
    #     """
    ...
    def __getitem__(self, item):
        ...
        # raw1, raw2 = self._fetch_item(item)
        raw1, raw2 = self.corpus[item]
        tokens_unmaked1 = self.tokenizer.tokenize(raw1)
        tokens_unmaked2 = self.tokenizer.tokenize(raw2)
        ids_unmask1 = self.tokenizer.convert_tokens_to_ids(tokens_unmaked1)
        ids_unmask2 = self.tokenizer.convert_tokens_to_ids(tokens_unmaked2)
        ...
        # print(ids, mlm_labels, ids_unmask1, ids_unmask2)
